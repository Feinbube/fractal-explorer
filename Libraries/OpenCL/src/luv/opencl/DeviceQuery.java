...
public class DeviceQuery {
    ...
    public static void main(String args[]) {
        System.out.println(new DeviceQuery().querySystemInformation());
    }

    public String querySystemInformation() {
        String result = "";
        ...
        for (int i = 0; i < platforms.length; i++) {
            result += queryPlatformInformation(platforms[i], devices);
        }

        // Print the infos about all devices
        for (cl_device_id device : devices) {
            result += queryDeviceInformation(device);
        }

        return result;
    }

    protected String queryPlatformInformation(cl_platform_id platform, List<cl_device_id> devices) {
        String result = "";
    ...
    protected String queryDeviceInformation(cl_device_id device) {
        String result = "";
        // CL_DEVICE_NAME
        String deviceName = getString(device, CL_DEVICE_NAME);
        result += "--- Info for device " + deviceName + ": ---" + "\n";
        result += String.format("CL_DEVICE_NAME: \t\t\t%s\n", deviceName);
        // CL_DEVICE_VENDOR
        String deviceVendor = getString(device, CL_DEVICE_VENDOR);
        result += String.format("CL_DEVICE_VENDOR: \t\t\t%s\n", deviceVendor);
        // CL_DRIVER_VERSION
        String driverVersion = getString(device, CL_DRIVER_VERSION);
        result += String.format("CL_DRIVER_VERSION: \t\t\t%s\n", driverVersion);
        // CL_DEVICE_TYPE
        long deviceType = getLong(device, CL_DEVICE_TYPE);
        if ((deviceType & CL_DEVICE_TYPE_CPU) != 0) {
            result += String.format("CL_DEVICE_TYPE:\t\t\t\t%s\n", "CL_DEVICE_TYPE_CPU");
        }
        ...
        if ((queueProperties & CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE) != 0) {
            result += String.format("CL_DEVICE_QUEUE_PROPERTIES:\t\t%s\n", "CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE");
        }
        ...
        result += String.format("CHAR %d, SHORT %d, INT %d, LONG %d, FLOAT %d, DOUBLE %d\n\n\n",

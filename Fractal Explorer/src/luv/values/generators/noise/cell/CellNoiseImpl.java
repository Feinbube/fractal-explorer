...
public class CellNoiseImpl implements Serializable {
     ...
     */
    private float DENSITY_ADJUSTMENT, DENSITY_ADJUSTMENT_INV;
    ...
    public CellNoiseImpl() {
        DENSITY_ADJUSTMENT = 0.398150f;
        DENSITY_ADJUSTMENT_INV = 1.0f / DENSITY_ADJUSTMENT;
    ...
    private void noise2D(CellDataStruct cd) {
        ...
        for (i = 0; i < cd.max_order; i++) {
            cd.F[i] = (float) Math.sqrt(cd.F[i]) * DENSITY_ADJUSTMENT_INV;
     ...
     * 2D
     */
    private void AddSamples(int xi, int yi, float[] at, CellDataStruct cd) {

...
public class HybridMultiply extends HybridValueMapper {

    public HybridMultiply(OpenCL openCL, ValueGenerator... generators) {
    ...
    protected void runKernel(HybridMemory output, double x0, double y0, double x1, double y1, int w, int h, HybridMemory... input) {
        kernel.run(w, h, 0, input[0], output, ((PojoMultiply)getPojoImplementation()).factor);
    }

    @Override
    protected ValueGenerator getNewPOJOImplementation() {
        return new PojoMultiply();
    }
}
